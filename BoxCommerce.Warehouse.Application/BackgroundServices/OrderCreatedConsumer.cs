...
using static BoxCommerce.Warehouse.Domain.Events.ProductOrderedInStockComponentsEvent;
...
namespace BoxCommerce.Warehouse.Application.BackgroundServices
{
    public class OrderCreatedConsumer : BackgroundService
    {
        ...
        {
            var newProducts = orderCreatedMessage.Products.Where(x => string.IsNullOrEmpty(x.ProductId)).Select(x => x.ProductId!).ToList();

            if (newProducts.Count == 0)
                return;

            using var scope = _services.CreateScope();
            var dbConnectionString = _configuration["ConnectionStrings:DbConnection"];
            using IDbConnection db = new SqlConnection(dbConnectionString);
            var dbContext = scope.ServiceProvider.GetRequiredService<IWarehouseDbContext>();

            foreach (var newProduct in orderCreatedMessage.Products)
            {
                ...
                var valuesClause = string.Join(",", components.Select((opt, index) => $"(@option{index})"));
...
", new { Status = ComponentStatus.READY });
                ...
                if (inStockComponents.Any())
                {
                    product.AddDomainEvent(new ProductOrderedInStockComponentsEvent(product, inStockComponents));
                }
